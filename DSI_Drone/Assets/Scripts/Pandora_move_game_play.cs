...
public class Pandora_move_game_play: MonoBehaviour
{
    Vector3 End_pos;
    Vector3 Start_pos;
    ...
    private string[][] s_texts =
    {
        ...
        new string[]{
            ...
            "There was a hidden garden in the Chaotic spot!",
        ...
        new string[]{
            ...
            "it is a mystery how that building came all the way from Italy to Pandora!",
        },
        new string[]
        {
            ...
            {//teleport to next stage
                ...
                else
                {
                    ...
                    if (Stage < 4)
                    {
                        ...
                        current_time = 0.0f;
                        print("next Stage");
                        Pandora_ControlPanelBlink.StageComplete = false;
                        transform.position = startPositions[Stage];
                        Start_pos = transform.position;
                        End_pos = transform.position;
                        Dir = 0;
                        transform.rotation = Quaternion.Euler(0, 0, 0); // initialize everything
                        if (stageCompleteEffect_played[Stage - 1])
                        {
                    ...
                    else
                    {
                        ...
                        writer.WriteLine("CorrectCount: " + Pandora_ControlPanelBlink.correctCount + " / TotalCount: " + Pandora_ControlPanelBlink.moveCount);
                        writer.Close();
                    }
                }
            }
    ...
    {
        bool result = false;
        result = Pandora_ControlPanelBlink.StageComplete;
        //check values in controlPanelBlink
        if (Pandora_ControlPanelBlink.stageResult[Stage])
        {
    ...
    IEnumerator SuccessMessage()
    {
        ...
        for (int i = 0; i < s_texts[Stage].Length; i++)
        {
            for (int j = 0; j <= s_texts[Stage][i].Length; j++)
            {
                GuideText.text = s_texts[Stage][i].Substring(0, j);
